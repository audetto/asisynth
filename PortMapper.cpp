...
namespace ASI
{
  ...
  PortMapper::PortMapper(jack_client_t * client, const bool simpleNames)
    : m_client(client), m_simpleNames(simpleNames), m_midiInputId(0), m_midiOutputId(0), m_audioInputId(0), m_audioOutputId(0)
  {
  }

  std::string PortMapper::getPortName(const char * port_name,
  ...
  jack_port_t * PortMapper::registerPort(const char * port_name,
					 const char * port_type,
					 const unsigned long flags)
  {
